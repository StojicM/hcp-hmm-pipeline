...
Loads the concatenated parcel time series, fits the configured HMM backend,
...
def stationary_distribution(P: numpy.ndarray, eps=1e-12):
    ...
    s = pi.sum()
    return (pi / s) if s >= eps else numpy.ones(K) / K
...
class HMMConfig:
    ...
    - `K`, `max_iter`, `seed`: HMM hyperparameters
    - `backend`: hmm implementation to use ("arhmm" | "slds")
...
class HMMRunner:
    ...
    def _estimate_means_covars(
        ...
        """Estimate state means and diagonal variances from posteriors."""
        K = int(probs.shape[1])
        P = int(X.shape[1])
        weights = probs
    ...
    def _export_from_fitted(
        ...
        if getattr(model, "means_", None) is None or getattr(model, "covars_", None) is None:
            means, covars = self._estimate_means_covars(X, probs_all)
            model.means_ = means
    ...
    def fit_and_export(self):
        ...
        backend = str(getattr(self.cfg, "backend", "arhmm")).lower().replace("-", "_")
        log.info(
            "hmm_train_begin",
            extra={
                "K": self.cfg.K,
        ...
        if backend == "arhmm":
            ...
            model = SLDS(
                n_states=int(self.cfg.K),
                latent_dim=int(getattr(self.cfg, "slds_latent_dim", 4)),
                max_iter=int(self.cfg.max_iter),
                seed=int(self.cfg.seed),
            )
        else:
            raise SystemExit(f"Unknown HMM backend '{backend}'. Use 'arhmm' or 'slds'.")
        model.fit(X, lengths)
        logL = model.score(X, lengths)
        self._export_from_fitted(model, X, idx, lengths, backend, float(logL))
